...
- **Scope:** 5-task bundled smoke set by default, with `--dataset`/`--instances` for custom SWE-bench Lite test-split subsets, `--trials` for repeated runs, and `--resume` for interrupted runs (see [docs/tier1-plan.md](./tier1-plan.md)). Still no config file or cloud execution.
